...
<head>
    ...
    <meta name="csrf-token" content="<?php echo e(csrf_token()); ?>">
    <title><?php echo $__env->yieldContent('title', 'Dashboard'); ?> - <?php echo e($companyName ?: config('app.name', 'Aronium Lite')); ?></title>
...
<div class="flex h-screen overflow-hidden">
    ...
    <aside class="flex flex-col bg-[#0a0f28] text-white shrink-0 transition-all duration-300 fixed lg:relative inset-y-0 left-0 z-40" :class="sidebarOpen ? 'w-56 translate-x-0 pointer-events-auto' : 'w-56 -translate-x-full lg:w-16 lg:translate-x-0 pointer-events-none lg:pointer-events-auto'">
        <div class="flex flex-col items-center justify-center h-20 px-2 border-b border-white/10 overflow-hidden">
            ...
            <template x-if="!companyLogo">
                <div class="w-8 h-8 rounded-full bg-blue-600 flex items-center justify-center font-bold text-sm flex-shrink-0 ring-2 ring-white/20"><?php echo e(Str::substr(config('app.name', 'AL'), 0, 1)); ?></div>
            </template>
            <span class="mt-1.5 text-xs font-semibold text-white/80 whitespace-nowrap truncate max-w-full" x-show="sidebarOpen" x-cloak x-text="companyName || '<?php echo e(config('app.name', 'Aronium Lite')); ?>'"></span>
        </div>
        ...
        <nav class="flex-1 py-4 space-y-1 px-2 overflow-y-auto">
            ...
            <?php $__currentLoopData = $navModules; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $key => $mod): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                <?php
                    $route = $moduleRoutes[$key] ?? null;
                    $minLevel = $mod['min_level'] ?? 0;
                    $show = true;
                ?>
                <?php if($route && isset($sidebarIcons[$key]) && $minLevel <= 9): ?>
                <a href="<?php echo e(route($route)); ?>" class="sidebar-nav-item flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm transition-colors <?php echo e(request()->routeIs($route) ? 'bg-blue-600/20 text-blue-400' : 'text-gray-400 hover:bg-white/5 hover:text-white'); ?>" title="<?php echo e($mod['label']); ?>" data-min-level="<?php echo e($minLevel); ?>">
                    <svg class="w-5 h-5 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor"><?php echo $sidebarIcons[$key]; ?></svg>
                    <span x-show="sidebarOpen" x-cloak class="whitespace-nowrap"><?php echo e($mod['label']); ?></span>
                </a>
                <?php endif; ?>
...
<?php /**PATH E:\POS Software\D Star Company\resources\views/layouts/app.blade.php ENDPATH**/ ?>