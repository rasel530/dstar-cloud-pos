...
<div x-data="loyaltyManager" >
    ...
    <div x-show="showAddCard" x-cloak class="fixed inset-0 z-50 flex items-center justify-center p-4" @click.self="showAddCard = false">
        ...
        <div class="relative bg-white dark:bg-[#1a1f3d] rounded-xl w-full max-w-md border border-gray-200 dark:border-white/10 shadow-2xl" @click.stop>
            <div class="px-6 py-4 border-b border-gray-200 dark:border-white/10 flex items-center justify-between">
                <h3 class="font-semibold text-gray-900 dark:text-white">Create Loyalty Card</h3>
                <button @click="showAddCard = false" class="p-1 rounded-lg hover:bg-gray-100 dark:hover:bg-white/5 text-gray-400">&times;</button>
            </div>
            <div class="p-6 space-y-4">
                <div>
                    <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Customer</label>
                    <select x-model="newCard.customer_id" class="w-full px-3 py-2 bg-white dark:bg-[#0f1535] border border-gray-300 dark:border-white/10 rounded-lg text-sm">
    ...
    <div x-show="showModal" x-cloak class="fixed inset-0 z-50 flex items-center justify-center p-4" @click.self="showModal = false">
        ...
        <div class="relative bg-white dark:bg-[#1a1f3d] rounded-xl w-full max-w-sm border border-gray-200 dark:border-white/10 shadow-2xl" @click.stop>
            <div class="px-6 py-4 border-b border-gray-200 dark:border-white/10 flex items-center justify-between">
                <h3 class="font-semibold text-gray-900 dark:text-white" x-text="transactionType === 'earn' ? 'Earn Points' : 'Redeem Points'"></h3>
                <button @click="showModal = false" class="p-1 rounded-lg hover:bg-gray-100 dark:hover:bg-white/5 text-gray-400 hover:text-gray-600">&times;</button>
            </div>
            <div class="p-6 space-y-4">
                <p class="text-sm text-gray-500 dark:text-gray-400" x-text="'Current balance: ' + (selectedCard?.points_balance || 0) + ' pts'"></p>
                <div>
                    <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Points</label>
                    <input type="number" x-model="points" min="1" class="w-full px-3 py-2 bg-white dark:bg-[#0f1535] border border-gray-300 dark:border-white/10 rounded-lg text-sm">
                </div>
