...
@section('content')
<div x-data="promotionsManager" x-init="init()" class="px-6">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 py-4">
        <div>
            ...
            <p class="text-sm text-slate-500 dark:text-slate-400 mt-1">Manage discounts and happy hours</p>
        </div>
        <button
            @click="openAdd()"
            ...
            <table class="min-w-full divide-y divide-slate-200 dark:divide-slate-700">
                <thead class="bg-slate-50 dark:bg-slate-800/50">
                    <tr>
                        ...
                        <th class="px-6 py-3.5 text-left text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider">Dates</th>
                        <th class="px-6 py-3.5 text-center text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider">Days Active</th>
                    ...
                    <template x-if="!loading && promotions.length === 0">
                        <tr>
                            <td colspan="5" class="px-6 py-16 text-center">
                                ...
                                <h3 class="mt-3 text-sm font-medium text-slate-900 dark:text-white">No promotions found</h3>
                                <p class="mt-1 text-sm text-slate-500 dark:text-slate-400">Get started by creating your first promotion.</p>
                            </td>
                        </tr>
                    </template>
                        ...
                        <tr class="hover:bg-slate-50 dark:hover:bg-slate-700/50 transition-colors">
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="flex items-center">
                                    ...
                                    <div class="ml-3">
                                        <div class="text-sm font-medium text-slate-900 dark:text-white" x-text="p.name"></div>
                                    </div>
                                </div>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="text-sm text-slate-600 dark:text-slate-300" x-text="formatDate(p.start_date)"></div>
                                <div class="text-xs text-slate-400 dark:text-slate-500" x-text="'to ' + formatDate(p.end_date)"></div>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-center">
                                <div class="flex items-center justify-center gap-1 flex-wrap">
                                    <template x-for="day in p.days_of_week || []" :key="day">
                                        <span class="inline-flex items-center px-1.5 py-0.5 rounded text-xs font-medium bg-slate-100 dark:bg-slate-700 text-slate-600 dark:text-slate-400" x-text="dayName(day).substring(0, 2)"></span>
                                    </template>
                                    <span x-show="!(p.days_of_week && p.days_of_week.length)" class="text-xs text-slate-400">All days</span>
                                </div>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-center">
                                <button
                                    ...
                                    :class="p.is_enabled ? 'bg-blue-600 dark:bg-blue-500' : 'bg-slate-300 dark:bg-slate-600'"
        ...
        <div class="flex min-h-full items-center justify-center p-4">
            ...
            >
                <div class="flex items-center justify-between px-6 py-4 border-b border-slate-200 dark:border-slate-700">
                    <h3 id="modal-title" class="text-lg font-semibold text-slate-900 dark:text-white" x-text="editing ? 'Edit Promotion' : 'Add Promotion'"></h3>
                    <button
                        @click="showModal = false"
                ...
                <form @submit.prevent="save()" class="px-6 py-4 space-y-4">
                    ...
                    <div x-show="editId">
                        ...
                        <div class="space-y-2 mb-3">
                            <template x-for="(item, idx) in promotionItems" :key="item.id">
                                <div class="flex items-center justify-between gap-2 px-3 py-2 bg-slate-50 dark:bg-slate-700/50 rounded-lg border border-slate-200 dark:border-slate-600">
                                    <div class="flex-1 min-w-0">
                                        <span class="text-sm text-slate-900 dark:text-white" x-text="item.product_name || getProductName(item.uid)"></span>
                                    ...
                                    <button
                                        ...
                                        class="p-1 text-slate-400 hover:text-red-600 dark:hover:text-red-400 rounded transition-colors flex-shrink-0"
                                    >
                                        <svg class="w-4 h-4" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" d="M6 18L18 6M6 6l12 12"/>
                                        </svg>
                            ...
                            <template x-if="promotionItems.length === 0">
                                <p class="text-xs text-slate-400 dark:text-slate-500">No products assigned yet.</p>
                            </template>
                        </div>
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-7 gap-2 items-end">
                            ...
                            <div class="sm:col-span-1">
                                <select
                                    ...
                                    class="w-full px-2 py-2 bg-white dark:bg-slate-700 border border-slate-300 dark:border-slate-600 rounded-lg text-sm text-slate-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 dark:focus:border-blue-500 transition-colors"
                                >
                            ...
                            <div class="sm:col-span-1">
                                <button
                                    ...
                                    class="w-full px-3 py-2 text-sm font-medium text-white bg-purple-600 hover:bg-purple-700 dark:bg-purple-500 dark:hover:bg-purple-600 rounded-lg transition-colors focus:outline-none focus:ring-2 focus:ring-purple-500 focus:ring-offset-2 dark:focus:ring-offset-slate-800 disabled:opacity-40 disabled:cursor-not-allowed"
                                >
                                    Add
                                </button>
                            </div>
                    ...
                    <div class="flex items-center gap-3 py-1">
                        <button
                            ...
                            class="relative inline-flex h-6 w-11 items-center rounded-full transition-colors duration-200 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 dark:focus:ring-offset-slate-800"
                            :class="form.is_enabled ? 'bg-blue-600 dark:bg-blue-500' : 'bg-slate-300 dark:bg-slate-600'"
                        ...
                        <span class="text-sm text-slate-700 dark:text-slate-300" x-text="form.is_enabled ? 'Enabled' : 'Disabled'"></span>
                    </div>
