...
@section('content')
<div x-data="usersManager" x-init="init()" class="px-6">
        ...
        <div class="overflow-x-auto">
            <table class="min-w-full divide-y divide-slate-200 dark:divide-slate-700">
                <thead class="bg-slate-50 dark:bg-slate-800/50">
                    <tr>
                        <th class="px-6 py-3.5 text-left text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider">Username</th>
                        ...
                        <th class="px-6 py-3.5 text-center text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider">Access Level</th>
                    ...
                    <template x-if="!loading && users.length === 0">
                        <tr>
                            <td colspan="5" class="px-6 py-16 text-center">
                                ...
                                <h3 class="mt-3 text-sm font-medium text-slate-900 dark:text-white">No users found</h3>
                                <p class="mt-1 text-sm text-slate-500 dark:text-slate-400">Get started by adding your first user.</p>
                            </td>
                        </tr>
                    </template>
                        ...
                        <tr class="hover:bg-slate-50 dark:hover:bg-slate-700/50 transition-colors">
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="flex items-center">
                                    ...
                                    <div class="ml-3">
                                        <div class="text-sm font-medium text-slate-900 dark:text-white" x-text="u.username"></div>
                                        <div class="text-xs text-slate-500 dark:text-slate-400" x-text="(u.first_name || '') + ' ' + (u.last_name || '')"></div>
                                    </div>
                                </div>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-slate-600 dark:text-slate-300" x-text="u.email"></td>
                            <td class="px-6 py-4 whitespace-nowrap text-center">
                                <span
                                    ...
                                    :class="{
                                        ...
                                        'bg-slate-100 dark:bg-slate-700 text-slate-600 dark:text-slate-400': u.access_level < 5,
                                    }"
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-center">
                                <button
                                    ...
                                    :class="u.is_enabled ? 'bg-blue-600 dark:bg-blue-500' : 'bg-slate-300 dark:bg-slate-600'"
        ...
        <div class="flex min-h-full items-center justify-center p-4">
            ...
            >
                <div class="flex items-center justify-between px-6 py-4 border-b border-slate-200 dark:border-slate-700">
                    <h3 id="modal-title" class="text-lg font-semibold text-slate-900 dark:text-white" x-text="editing ? 'Edit User' : 'Add User'"></h3>
                    <button
                        @click="showModal = false"
                ...
                <form @submit.prevent="save()" class="px-6 py-4 space-y-4">
                    ...
                    <div x-show="form.branch_id === ''">
                        ...
                        <div class="grid grid-cols-2 gap-1.5 max-h-32 overflow-y-auto border border-slate-200 dark:border-slate-600 rounded-lg p-2">
                            <template x-for="b in branches" :key="'mb' + b.id">
                                <label class="flex items-center gap-1.5 text-xs cursor-pointer hover:bg-slate-50 dark:hover:bg-slate-600 px-1.5 py-1 rounded">
